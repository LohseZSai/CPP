...
def calculate_pattern(guess, true):
    ...
    (0, 2, 0, 2, 2)
    """
    wrong = [i for i, v in enumerate(guess) if v != true[i]] #统计猜测单词的位置和内容，如果位置上不是答案对应字母就放入
    counts = Counter(true[i] for i in wrong)#此处是统计每个元素出现了几次，返回的是字典，第一次见到欸
    pattern = [2] * 5      #默认全为绿色
    for i in wrong:              
        v = guess[i]                               #
        if counts[v] > 0:  #在答案中有该字母，return1（橙色）
            pattern[i] = 1 #相应位置return橙色
            counts[v] -= 1   #避免出现多次匹配，就是单词中有重复字母的情况
        else:
            pattern[i] = 0
...
def generate_pattern_dict(dictionary):
    ...
    for word in tqdm(dictionary):    #设置进度条反映进度，非常good，可以学以致用
