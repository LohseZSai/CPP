...
def GS(mat,result_column,eps):
    ...
    G = np.dot(np.linalg.inv((matD - matR)), matL)
    b = np.dot(np.linalg.inv((matD - matR)), result_column)
    #求D矩阵的逆矩阵
    results = []
    times = 0
    ...
    if(max(eigenvalues) < 1):print("g-s法得出该矩阵收敛")
    else: print("g-s法得出该矩阵收敛")
    while(times <= 30):
        times += 1;
